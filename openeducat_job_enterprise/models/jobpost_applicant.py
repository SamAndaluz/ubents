...
class JobStage(models.Model):
    ...
    def default_get(self, fields):
        if self._context and self._context.get('default_post_id') and not self._context.get('job_post_stage_mono', False):
...
class OpJobApplicant(models.Model):
    ...
    def _default_stage_id(self):
        if self._context.get('default_stage_id'):
            return self.env['job.post.stage'].search([ '|',
                ...
                ('fold', '=', False)], order='sequence asc', limit=1).id
        return False
    ...
    stage_id = fields.Many2one('job.post.stage', 'Stage', ondelete='restrict', tracking=True,
