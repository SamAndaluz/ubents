...
class WebsitePlacementOffer(http.Controller):
    ...
    @http.route(
        '/activity/announcement/detail/<model("op.activity.announcement"):'
        'activity>',
        type='http', auth='public', website=True)
    def Activity_detail(self, activity, **kwargs):
    ...
    def Web_activity_detail(self, **kwargs):
        activity = request.env['op.activity.announcement'].search([])
...
class ActivityAnnouncementController(OnlineJob):
    ...
    @http.route([
        '/job_post/detail/<model("op.activity.announcement"):'
        'activity>/<model("op.job.post"):job_post_id>'],
        type='http', auth="public", website=True)
    def jobpost_detail(self, activity, job_post_id, **kwargs):
        return request.render("openeducat_placement_job_enterprise."
                              "job_post_detail",
                              {
                                  'activity': activity,
